						...
						if (req.body.password == entry.password) {
							networkDb.updateEntryWithAuthToken(req.body.username, authToken, function(authErr, updated) {
								if (authErr) {
									...
									// below is a custom c script 

									// var worker = exec('wol ' + entry['mac-address'], function(error, stdout, stderr) {
									// 	console.log(stdout);
									// 	if (error != null) {
									// 		console.log('exec error: ' + error);
									// 	}
									// });
									// wol.wake(entry['mac-address'], function(err) { if (err) { console.log(err); } else { return }});
									request.post({
