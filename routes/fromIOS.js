	...
	networkDb.findEntryByUsername(req.body.username, function(err, entry) {
		if (err) {
			...
			if (req.body.authToken == entry.authToken) {
				...
				connection.on('ready', function() {
					debug('Succesfully connected via SSH to host!');
					connection.exec('cd ' + req.body.filepath + ' && dirs=$(find . -maxdepth 1 -type d) && del="." && dirs=( "${dirs[@]/$del}" ) && files=$(find . -maxdepth 1 -type f) && arr=() && sub="./" && rep1="==" && rep2="=" && dirs="${dirs[*]//$sub/$rep1}" && files="${files[*]//$sub/$rep2}" && arr=( "${dirs[*]}" "\n" "${files[*]}" ) && echo "${arr[*]}"', function(execErr, stream) {
						if (err) {
							console.log('ERROR!');
							console.log(err);
							res.status(500).send({
