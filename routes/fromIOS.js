		...
		if (err) {
			...
			if (req.body.authToken == entry.authToken) {
				...
				connection.on('ready', function() {
					...
					var uploadScript = 'filename="' + filepath + '" && space=" " && escaped="\ " && filepath="${filename/$space/$escaped}" && username="' + req.body.username + '" && encoded_file="$(cat ${filepath} | base64)" && echo \'username=\'"${username}"\'&filepath=\'"${filepath}"\'&contents=\'"${encoded_file}"\' | curl -d @- \'http://pickup.azurewebsites.net/upload\'';
